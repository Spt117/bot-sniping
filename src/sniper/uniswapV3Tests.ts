...
export async function getEth(addr: string, addr2: string, wallet: GetTransaction) {
...
export async function checkPool(addr: string, addr2: string, wallet: GetTransaction) {
    const tokenData = await fetchTokenData(addr, wallet);
    const wethData = await fetchTokenData(addr2, wallet);
    ...
    for (let i = 0; i < fees.length; i++) {
        const currentPoolAddress = computePoolAddress({
            ...
            tokenB: new Token(wethData.chainId, wethData.address, wethData.decimals, wethData.symbol, wethData.name),
            fee: fees[i] as unknown as FeeAmount,
            // fee: FeeAmount.MEDIUM,
        });

        const code = await wallet.getWallet()?.provider?.getCode(currentPoolAddress);
        if (code && code !== "0x") {
